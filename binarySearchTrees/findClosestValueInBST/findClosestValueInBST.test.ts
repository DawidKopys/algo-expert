...
import {
  findClosestValueInBst as sol1,
  BST
} from './findClosestValueInBST.sol1'
import { findClosestValueInBst as sol2 } from './findClosestValueInBST.sol2'

const solutions = [sol1, sol2]

solutions.forEach((sol, idx) => {
  test(`findClosestValueInBst sol${idx + 1}`, async (t) => {
    ...
    await t.test(
      'returns the closest value when the target does not exist in the tree',
      () => {
        const tree = new BST(10)
        tree.left = new BST(5)
        tree.right = new BST(15)
